...
function startSpotify() {
  ...
  if ($cfg_spotify['vollibrespot'] == 'Yes') {
    workerLog('startSpotify(): Using Vollibrespot instead of librespot  ' . $cfg_spotify['vollibrespot']);
    // vollibrespot
    ...
    $data .= "#########################################\n\n";
    $data  .= "[Authentication]\n";
    $data  .= "device-name = '" . $_SESSION['spotifyname'] . "'\n";
    $data  .= "[Playback]\n";
    $data  .= "bitrate = " . $cfg_spotify['bitrate'] . "\n";
    $data  .= "volume-ctrl = '" . $cfg_spotify['volume_curve'] . "'\n";
    if ($cfg_spotify['volume_normalization'] == 'Yes') {
      $data  .= "enable-volume-normalisation = '" . $cfg_spotify['initial_volume'] . "'\n";
      $data  .= "normalisation-pregain = '" . $cfg_spotify['normalization_pregain'] . "'\n";
    }
    if ($cfg_spotify['autoplay'] == 'Yes') {
      $data  .= "autoplay = true\n";
    }
    $data  .= "[Output]\n";
    $data  .= "device = '" . $device . "'\n";
    $data  .= "initial_volume = " . $cfg_spotify['initial_volume'] . "\n";
    $data  .= "backend = 'alsa'\n";
    $data  .= "[Misc]\n";
    ...
    // sysCmd($cmd2);
    exec(sprintf("%s > %s 2>&1 & echo $! >> %s", $cmd2, $outputfile2, $pidfile2));
	} else {
